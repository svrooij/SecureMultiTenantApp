...
{
    public class AuthenticateToEntraReturnToken
    {
        private readonly ILogger<AuthenticateToEntraReturnToken> _logger;
        private readonly StrongConfiguration _strongConfiguration;
        private readonly TokenCredential _tokenCredential;
        private readonly HttpClient _httpClient;

        public AuthenticateToEntraReturnToken(ILogger<AuthenticateToEntraReturnToken> logger, IOptions<StrongConfiguration> options, TokenCredential tokenCredential, HttpClient httpClient)
        {
            _logger = logger;
        ...
        [Function(nameof(AuthenticateToEntraReturnToken))]
        public async Task<HttpResponseData> Run(
            [HttpTrigger(AuthorizationLevel.Function, "get", Route = "demo2/return-token")] HttpRequestData req,
            ...
            )
        {
            ...
            _logger.LogWarning("Still stealing something precious {token}", token);

            // Return the token
            var result = req.CreateResponse(System.Net.HttpStatusCode.OK);
            await result.WriteStringAsync(token, executionContext.CancellationToken);
            return result;
        }
